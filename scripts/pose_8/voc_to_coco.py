    ...
    for line in list_fp:
       ...
       # print(line)
        ...
        for obj in get(root, 'object'):
            ...
            x5 = float(get_and_check(keypoints, 'x5', 1).text) 
            y5 = float(get_and_check(keypoints, 'y5', 1).text)
            x5f = float(get_and_check(keypoints, 'x5f', 1).text)
            x6 = float(get_and_check(keypoints, 'x6', 1).text)
            y6 = float(get_and_check(keypoints, 'y6', 1).text)
            x6f = float(get_and_check(keypoints, 'x6f', 1).text)
            x7 = float(get_and_check(keypoints, 'x7', 1).text)
            y7 = float(get_and_check(keypoints, 'y7', 1).text)
            x7f = float(get_and_check(keypoints, 'x7f', 1).text)
            x8 = float(get_and_check(keypoints, 'x8', 1).text) 
            y8 = float(get_and_check(keypoints, 'y8', 1).text)
            x8f = float(get_and_check(keypoints, 'x8f', 1).text)
            ...
            ann = {'area': o_width*o_height, 'iscrowd': 0, 'image_id':
                   ...
                   'keypoints':[x1,y1,x1f, x2,y2,x2f, x3,y3,x3f, x4,y4,x4f, x5,y5,x5f, x6,y6,x6f, x7,y7,x7f, x8,y8,x8f],
                   'category_id': category_id, 'id': bnd_id, 'ignore': 0,
                   'num_keypoints': 4}
            json_dict['annotations'].append(ann)
            bnd_id = bnd_id + 1
...
if __name__ == '__main__':
    ...
    xml_dir = "/workspace/zigangzhao/0116_Card/all/xml_test"
