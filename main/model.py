...
from nets.basemodel import resnet50, resnet_arg_scope, resnet_v1, mobilenet_v2_224, mobilenet_v2_arg_scope
...
class Model(ModelDesc):
    ...
    def head_net(self, blocks, is_training, trainable=True):
        ...
        with slim.arg_scope(mobilenet_v2_arg_scope(cfg.weight_decay, is_training=True, depth_multiplier=1.0, regularize_depthwise=False,dropout_keep_prob=1.0)):
    ...
    def make_network(self, is_train):
        ...
        #resnet_fms = backbone(image, is_train, bn_trainable=True)  ###gaidong

        moblienetv2_fms = backbone(image)
        # print(moblienetv2_fms.shape)
        heatmap_outs = self.head_net(moblienetv2_fms, is_train)
