    ...
    while img_start < det_range[1]:
        ...
        for i in range(len(kps_result)):
            if cfg.dataset == 'COCO':
                ...
                #               keypoints=kps_result[i].round(3).tolist())
                result = dict(image_id=im_info['image_id'], keypoints=kps_result[i].round(3).tolist(), category_id=0, score=0.99)
...
def test(test_model):
    ...
    # human bbox load
    if cfg.useGTbbox== False and cfg.testset in ['train', 'test']:
