...
sys.path.append("/workspace/zigangzhao/Pose_8_IDCard/")
from data.MPII.dataset import Dataset
from lib.tfflat.data_provider import DataFromList, MultiProcessMapDataZMQ, BatchData, MapData
from main.gen_batch import generate_batch
...
class Base(object):
    ...
    def load_weights(self, model=None, test=False):
        ...
        #         # restore graph
        #         saver = tf.train.import_meta_graph("/workspace/zigangzhao/TF-SimpleHumanPose/data/imagenet_weight_mobilenetv2/mobilenet_v2_1.4_224.ckpt.meta")
        #         #restore ckpt
        #         saver.restore(sess, "/workspace/zigangzhao/TF-SimpleHumanPose/data/imagenet_weight_mobilenetv2/mobilenet_v2_1.4_224.ckpt")
        #         load_model(self.sess, saver)
                

        if isinstance(model, int) and not test:
            model = os.path.join(self.cfg.model_dump_dir, 'snapshot_%d.ckpt' % model)
            # os.path.join(self.cfg.model_dump_dir_test, 'snapshot_%d.ckpt' % model)

        if isinstance(model, int) and test:
            print("==========test========")
            model = os.path.join(self.cfg.model_dump_dir_test, 'snapshot_%d.ckpt' % model)#os.path.join(self.cfg.model_dump_dir_test, 'snapshot_%d.ckpt' % model)
...
class Trainer(Base):
    ...
    def train(self):
        ...
        #self.load_weights(model='pretrain')
        ...
        for itr in range(start_itr, end_itr):
            ...
            if itr % self.itr_per_epoch == 0 and itr > 3000:
...
class Tester(Base):
    ...
    def _make_data(self):
        ...
        if self.cfg.multi_thread_enable:
            ...
            data_load_thread = MapData(data_load_thread, generate_batch)
        data_load_thread = BatchData(data_load_thread, self.cfg.test_batch_size)

        data_load_thread.reset_state()
        dataiter = data_load_thread.get_data()
        print(len(train_data))
        print(self.cfg.batch_size)
        return dataiter, math.ceil(len(train_data)/self.cfg.test_batch_size/self.cfg.num_gpus) 
    ...
    def predict_one_1(self, data=None):
        # TODO(reduce data in limited batch)
        assert len(self.summary_dict) == 0, "still not support scalar summary in testing stage"
        setproctitle.setproctitle('test epoch:' + str(self.cur_epoch))
        ...
        self.gpu_timer.tic()
        train_saver = Saver(self.sess, tf.global_variables(), self.cfg.mudel_dump_dir1)
        model_path = "/workspace/zigangzhao/Pose_8_IDCard/main/pose_ckpt/snapshot_180.ckpt"
        print("=====starting restored===")
        net_saver = tf.train.Saver()
        res = self.sess.run([*self.graph_ops, *self.summary_dict.values()], feed_dict=feed_dict)               
        net_saver.restore(self.sess, model_path)
        train_saver.save_model(180)
        print("=====restored finished===")
